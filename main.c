
#include "ft_printf.h"
#include <locale.h>
#include <wchar.h>
#define BLURB "Authentic imitation!"


int main()
{
	setlocale(LC_ALL, "");

//	printf("\norigin_return = %d", printf("|%05.2%|") - 1);
//	printf("\n");
//	printf("%i\n", MB_CUR_MAX);
	wchar_t *str = NULL;


//    printf("\n");
//    printf("%s", NULL);
//	  ft_printf("|%10.3s|", NULL);
//    printf("\norigin_return = %d", printf("\n|%10.3s|", NULL) - 1);

//	printf("\n");
//	printf("%s\n", NULL);
//	ft_printf("%s\n", NULL);
//	printf("\norigin_return = %d", printf("\n%s\n", NULL) -1);


//

//		printf("|%ld|\n", 789789789789);
//	 ft_printf("|%ld|\n", 789789789789);




//
//	  ft_printf("%%");
//	  printf("\n");
//	  printf("%%");
//	 printf("\norigin_return = %d", printf("%%"));
//
//		 printf("\n");
//
//		 ft_printf("\\");
//		 printf("\n");
//		printf("\norigin_return = %d", printf("\\"));
//
//		 printf("\n");
	//
//		 ft_printf("%*.*s", 0, 3, "1234");
//		 printf("\n");
//		 printf("\norigin_return = %d", printf("%*.*s", 0, 3, "1234"));
	//
	//
//		 ft_printf("|%-*.*s|", 10, 3,"LIMON");
//		 printf("\n");
//		printf("\norigin_return = %d", printf("|%-*.*s|", 10, 3,"LIMON"));
//
//		 printf("\n");
	//
//		ft_printf("%*s", 3,"LIMON");
//		printf("\n");
//		printf("\norigin_return = %d", printf("%*s", 3,"LIMON"));
//
//		printf("\n");
	//
//	 	ft_printf("%-*.*s", 3, 10,"LIMON");
//	 	printf("\n");
//
//	 	printf("\norigin_return = %d", printf("%-*.*s", 3, 10,"LIMON"));
//
//	 	printf("\n");
//
//		ft_printf("|%-*.*s|", -10, 3,"LIMON");
//		printf("\n");
//		printf("\norigin_return = %d", printf("|%-*.*s|", -10, 3,"LIMON"));
//
//		printf("\n");
//
//		 ft_printf("|%*.*s|", 10, 3,"LIMON");
//		 printf("\n");
//		printf("\norigin_return = %d", printf("|%*.*s|", 10, 3,"LIMON"));
//
//		 printf("\n");
	//
//		ft_printf("%10.3s", "LIMON");
//		printf("\n");
//		printf("%10.3s", "LIMON");
//		printf("\norigin_return = %d", printf("%10.3s", "LIMON"));
//
//		printf("\n");
	//
//		ft_printf("%10.*s", 3, "LIMON");
//		printf("\n");
//		printf("%10.*s", 3, "LIMON");
//		printf("\norigin_return = %d", printf("%10.*s", 3, "LIMON"));
//		printf("\n");
	//
//		ft_printf("%10.3s", "LIMON");
//		printf("\n");
//		printf("%10.3s", "LIMON");
//		printf("\norigin_return = %d", printf("%10.3s", "LIMON"));
//		printf("\n");
	//
//		ft_printf("Ello my frieands");
//		printf("\n");
//		printf("Ello my frieands");
//		printf("\norigin_return = %d", printf("\nEllo my frieands"));
//		printf("\n");
	//
//		ft_printf("123 %4.s", "abc");
//		printf("\n");
//		printf("123 %4.s", "abc");
//		printf("\norigin_return = %d", printf("\n123 %4.s", "abc"));
//		printf("\n");
	//
//

//		printf("\norigin_return = %d", printf("[%2s]", BLURB));
//		ft_printf("[%2s]", BLURB);
//		printf("\n");
	//
//		ft_printf("[%24s]", BLURB);
//		printf("\norigin_return = %d", (printf("\n[%24s]", BLURB)) - 1);
//		printf("\n");
	//
//		ft_printf("[%24.5s]", BLURB);
//		printf("\n");
//		printf("\norigin_return = %d", printf("\n[%24.5s]", BLURB) - 1);
//		printf("\n");
	//
//		ft_printf("The color: %s", "blue");
//		printf("\n");
//		printf("The color: %s", "blue");
//		printf("\norigin_return = %d", printf("\nThe color: %s", "blue") - 1);
//		printf("\n");
	//
//		ft_printf("[%-24.5s]", BLURB);
//		printf("\n");
//		printf("[%-24.5s]", BLURB);
//		printf("\norigin_return = %d", printf("\n[%-24.5s]", BLURB) - 1);
//		printf("\n");
	//
//		ft_printf(":%s:", "Hello, world!");
//		printf("\n");
//		printf(":%s:", "Hello, world!");
//		printf("\norigin_return = %d", printf("\n:%s:", "Hello, world!") - 1);
//		printf("\n");
	//
//		ft_printf(":%15s:", "Hello, world!");
//		printf("\n");
//		printf(":%15s:", "Hello, world!");
//		printf("\norigin_return = %d", printf("\n:%15s:", "Hello, world!") - 1);
//		printf("\n");
	//
//		ft_printf(":%.10s:", "Hello, world!");
//		printf("\n");
//		printf(":%.10s:", "Hello, world!");
//		printf("\norigin_return = %d", printf("\n:%.10s:", "Hello, world!") - 1);
//		printf("\n");
	//
//		ft_printf(":%-10s:", "Hello, world!");
//		printf("\n");
//		printf(":%-10s:", "Hello, world!");
//		printf("\norigin_return = %d", printf("\n:%-10s:", "Hello, world!") - 1);
//		printf("\n");
	//
//		ft_printf(":%-15s:", "Hello, world!");
//		printf("\n");
//		printf(":%-15s:", "Hello, world!");
//		printf("\norigin_return = %d", printf("\n:%-15s:", "Hello, world!") - 1);
//		printf("\n");
	//
//		ft_printf(":%.15s:", "Hello, world!");
//		printf("\n");
//		printf(":%.15s:", "Hello, world!");
//		printf("\norigin_return = %d", printf("\n:%.15s:", "Hello, world!") - 1);
//		printf("\n");
	//
//		ft_printf(":%15.10s:", "Hello, world!");
//		printf("\n");
//		printf(":%15.10s:", "Hello, world!");
//		printf("\norigin_return = %d", printf("\n:%15.10s:", "Hello, world!") - 1);
//		printf("\n");
	//
//		ft_printf(":%-15.10s:", "Hello, world!");
//		printf("\n");
//		printf(":%-15.10s:", "Hello, world!");
//		printf("\norigin_return = %d", printf("\n:%-15.10s:", "Hello, world!") - 1);
//		printf("\n");
	//

//		printf("\n");
//		printf("%.3s", "blue");
//		ft_printf("%.3s", "blue");
//		printf("\norigin_return = %d", printf("\n%.3s", "blue") - 1);
//		printf("\n");
	//
//		printf("\n");
//		printf("%2.3s", "blue");
//		ft_printf("%2.3s", "blue");
//		printf("\norigin_return = %d", printf("\n%2.3s", "blue") - 1);
//		printf("\n");
	//
//		printf("\n");
//		printf("%4.3s", "blue");
//		ft_printf("%4.3s", "blue");
//		printf("\norigin_return = %d", printf("\n%4.3s", "blue") - 1);
//		printf("\n");
	//
//		printf("\n");
//		printf("%3.3s", "blue");
//		ft_printf("%3.3s", "blue");
//		printf("\norigin_return = %d", printf("\n%3.3s", "blue") - 1);
//		printf("\n");
	//
//		printf("\n");
//		printf("%.3s", "bl");
//		ft_printf("%.3s", "bl");
//		printf("\norigin_return = %d", printf("\n%.3s", "bl") - 1);
//		printf("\n");
	//
//		printf("\n");
//		printf("%5.7s", "blue");
//		ft_printf("%5.7s", "blue");
//		printf("\norigin_return = %d", printf("\n%5.7s", "blue") - 1);
//		printf("\n");
	//
//		printf("\n");
//		printf("%2.5s", "blue");
//		ft_printf("%2.5s", "blue");
//		printf("\norigin_return = %d", printf("\n%2.5s", "blue") - 1);
//		printf("\n");
	//
//		ft_printf("%7.3s", "blue");
//		printf("\n");
//		printf("%7.3s", "blue");
//		printf("\norigin_return = %d", printf("\n%7.3s", "blue") - 1);
//		printf("\n");
	//

//	ft_printf("%.s", "blue");
//	printf("\n");
//	printf("%.s", "blue");
//	printf("\norigin_return = %d", printf("%.s", "blue"));
//	printf("\n");



//		printf("\n");
//		printf("%2.3s", "blue");
//		ft_printf("%2.3s", "blue");
//		printf("\norigin_return = %d", printf("\n%2.3s", "blue") - 1);
//		printf("\n");
	//

//		ft_printf("|%7.7s|", "blue");
//		printf("\norigin_return = %d", printf("\n|%7.7s|", "blue") - 1);
//		printf("\n");
	//

//		ft_printf("|%4.5s|", "blue");
//		printf("\norigin_return = %d", printf("\n|%4.5s|", "blue") - 1);
//		printf("\n");
	//
//		ft_printf("|%-10s|", "blue");
//		printf("\norigin_return = %d", printf("\n|%-10s|", "blue") - 1);
//		printf("\n");
	//
//		printf("\n");
//        ft_printf("%-10.10s", "blue");
//		printf("%-10.10s", "blue");
//		printf("\norigin_return = %d", printf("\n%-10.10s", "blue") - 1);
//		printf("\n");
	//
//		printf("\n");
//		printf("%-3.5s", "blue");
//		ft_printf("%-3.5s", "blue");
//		printf("\norigin_return = %d", printf("\n%-3.5s", "blue") - 1);
//		printf("\n");

//		printf("\n");
//        ft_printf("%-3.2s", "blue");
//		printf("%-3.2s", "blue");
//		printf("\norigin_return = %d", printf("\n%-3.2s", "blue") - 1);
//		printf("\n");
	//
//		printf("\n");
//        ft_printf("%-2.2s", "blue");
//		printf("%-2.2s", "blue");
//		printf("\norigin_return = %d", printf("\n%-2.2s", "blue") - 1);
//		printf("\n");
	//
//	ft_printf("|%3.10 1234%|");
//	printf("\n");
//    ft_printf("|%3c|\n", 0);
//    printf("|%3c|\n", 0);


//	printf("|%-3lc|\n", L'и');
//	ft_printf("|%3lc|\n", L'и');

//	int i = printf("|%10d|", -23);
//	printf("\n");
//	printf("%d\n", i - 2);
//	ft_printf("|%+-10.d|", 123);

//	ft_printf("|%-7d|", 123);
//
//	      printf("|%10.14ji|", 789789789789789789);
//	printf("\n");
//       ft_printf("|%10.14ji|", 789789789789789789);
//	printf("\n");
//	      printf("|%10.14ji|", 123);
//	printf("\n");
//	   ft_printf("|%10.14ji|", 123);
//	   printf("\n");
//	      printf("|%10.14ji|", 123);
//	printf("\n");
//	   ft_printf("|%10.14ji|", 123);
//    printf("\n");
//	      printf("|%10.14ji|", -123);
//	printf("\n");
//	   ft_printf("|%10.14ji|", -123);


//	printf("\n");
//
//    ft_printf("|%-05% OK %s OK %d OK %i OK %x OK %s|", "elo", 123, -123, 255, "ok");
//    printf("\n");
//    printf("\norigin_return = %d", printf("\n|%-05% OK %s OK %d OK %i OK %x OK %s|", "elo", 123, -123, 255, "ok") -1);

	int x = 3;
    ft_printf("%#x", &x);
	printf("%#x", &x);
	return (0);
}